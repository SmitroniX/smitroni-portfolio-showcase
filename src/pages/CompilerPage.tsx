...
import { Play, RotateCcw, Copy, Check, Terminal, Code2, Clock, Cpu, ArrowLeft, Trash2, Sparkles, ChevronDown, FileCode2, Sliders, AlertCircle, HelpCircle, CornerDownLeft, BookOpen, Send } from 'lucide-react';
...
export const CompilerPage: React.FC<CompilerPageProps> = ({ onBackToHome }) => {
  ...
  const [stdin, setStdin] = useState<string>(currentSamples[0]?.defaultStdin || '5');
  
  // Output & interactive terminal state
  const [output, setOutput] = useState<string>(
    '=== SmitroniX Cloud Compiler Ready ===\n' +
    'Click "Run" or press ⌘+Enter to execute.\n' +
    'You can also type inputs directly into the interactive terminal prompt below!\n'
  );
  const [interactiveInput, setInteractiveInput] = useState<string>('5');
  ...
  const selectLanguage = (lang: LanguageOption) => {
    ...
    if (firstSample) {
      ...
      setInteractiveInput(firstSample.defaultStdin || '5');
    }
    setOutput(`Switched compiler to ${lang.name} (${lang.version}).\nSample loaded: "${firstSample?.title || 'Default'}".\nClick "Run" to compile and execute.`);
  ...
  const selectSample = (sample: CodeSample) => {
    ...
    setInteractiveInput(sample.defaultStdin || '5');
    setOutput(`Loaded sample: "${sample.title}"\n${sample.description}\n\nClick "Run" to execute.`);
    setExecutionStats(null);
  ...
  const handleResetCode = () => {
    ...
    setInteractiveInput(selectedSample.defaultStdin || '5');
    setOutput('Editor reset to current sample code.');
    setExecutionStats(null);
  };
  ...
  const executeCode = async (overrideStdin?: string) => {
    ...
    let effectiveStdin = overrideStdin !== undefined ? overrideStdin : stdin;

    // Fallback: If code requires input and stdin is completely blank, supply sensible default
    if (requiresInput && !effectiveStdin.trim()) {
      effectiveStdin = interactiveInput.trim() || '5';
      setStdin(effectiveStdin);
    }
  ...
  const handleInteractiveInputSubmit = (e: React.FormEvent) => {
    ...
    setStdin(interactiveInput);
    executeCode(interactiveInput);
  };
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#070B13] text-slate-100 font-sans overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        ...
        {/* RIGHT PANE: Interactive Terminal Output Console */}
        <div
          ...
          <div className="border-t border-white/10 bg-[#070C15] p-3 shrink-0">
            <form onSubmit={handleInteractiveInputSubmit} className="space-y-2">
              <div className="flex items-center justify-between text-[11px] font-mono text-slate-400">
                ...
                <span className="text-slate-500 hidden sm:inline">Type value and press Enter</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 flex items-center bg-black/70 border border-white/15 rounded-xl px-3 py-1.5 focus-within:border-[#22C55E] transition-colors">
                  ...
                  <input
                    ...
                    placeholder="Enter input (e.g. 5, 10.5, text)..."
